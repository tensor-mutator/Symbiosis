...
class AGZChessNet(NetworkBaseAGZ):
      ...
      def __init__(self, state_shape: Tuple[int, int, int], action_size: int, **params) -> Pipeline:
          return Pipeline(meta_X=dict(shape=state_shape, dtype=tf.float32), meta_y=[dict(shape=action_size, dtype=tf.float32),
                                                                                    dict(shape=1, dtype=tf.float32)),
